...
def computer_select():
    hands = ["가위", "바위", "보"]
    computer_selected_hand = random.choice(hands)
    return computer_selected_hand
...
def determine_winner(user_choice, computer_choice):
    if user_choice == computer_choice:
        return 2
    if (user_choice == "가위" and 
        computer_choice == "보" or 
        user_choice == "바위" and 
        computer_choice == "가위" or 
        user_choice == "보" and 
        computer_choice == "바위"):
        return 1
    return 3
...
# -> 추가 입력
debug = False
while computer_win < 3 and human_win < 3:
    while True:
        user_choice = human_input()
        if user_choice != None:
            break
    # 컴퓨터 선택
